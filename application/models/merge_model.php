...
class Merge_model extends CI_Model
{
	...
	public function merge_cotel_preview($options){
		...
		$query = "select urn,telephone_id,telephone_number,t.description,ctps from companies inner join company_telephone t using(company_id) where urn ='".$options['source']."' and telephone_number not in(select telephone_number from companies inner join company_telephone using(company_id) where urn = '".$options['target']."')";
			$new_rows = $this->db->query($query)->result_array();
			foreach($new_rows as $newrow){
				foreach($newrow as $k=>$v){
				 if(empty($v)){
