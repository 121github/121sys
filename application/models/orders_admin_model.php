...
class Orders_admin_model extends CI_Model {
	...
	public function get_category($id){
		$this->db->where("item_category_id",$id);	
		return $this->db->get("flexicart_item_categories")->row_array();
		}
	public function get_item_categories(){
		$this->db->select("flexicart_item_categories.item_category_id id,item_category_name name, if(`count` is null,'0',`count`) `count`",false);
			$this->db->join("(select item_category_id,count(*) count from flexicart_item_subcategories group by item_category_id) b","b.item_category_id=flexicart_item_categories.item_category_id","LEFT",FALSE);
	$this->db->group_by("flexicart_item_categories.item_category_id");	
		return $this->db->get("flexicart_item_categories")->result_array();	
	}
	...
	public function get_item_subcategories($id=false){
		if($id){
		$this->db->where("item_category_id",$id);
		}
		return $this->db->get("flexicart_item_subcategories")->result_array();	
	}
	public function save_category($data){
		if(!empty($data['item_category_id'])){
		$this->db->where("item_category_id",$data['item_category_id']);
		$this->db->update("flexicart_item_categories",$data);	
		return $data['item_category_id'];
		} else {
		$this->db->insert("flexicart_item_categories",$data);	
