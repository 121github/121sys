...
function get_items($filter)
	{
		...
		$qry = "select * from flexicart_items where 1 $where";
	...
	function demo_update_loaded_cart_data()
	{
		...
		foreach($this->flexi_cart->cart_item_array() as $row_id => $column)
		{
			...
			$query = $this->db->get_where('items', $sql_where);
	...
	function demo_save_order()
	{
		...
		*/	
		...
		if ($this->input->post('checkout'))
		{
			...
			*/
			$custom_summary_data = array('urn'=>$_SESSION['record_urn'],'user_id'=>$_SESSION['user_id']);
