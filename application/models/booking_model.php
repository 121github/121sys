...
class Booking_model extends CI_Model
{
    ...
    public function get_events($start,$end,$attendee,$status) {
		...
		$query = "select appointment_id id,title, start, end, status, text, group_concat(distinct user_id separator ',') attendees, icon, appointment_type from appointments left join appointment_attendees using(appointment_id) join records using(urn) left join appointment_types using(appointment_type_id) where 1 ";
		$query .= $where;
