...
public function search_by_contact_phone($phone){
	//GHS ONLY
    $qry = "select urn,parked_code,urgent, name, outcome,fullname,source_name from records join contacts using(urn) join contact_telephone using(contact_id) left join record_details using(urn) left join data_sources on records.source_id = data_sources.source_id left join outcomes using(outcome_id) left join ownership using(urn) left join users using(user_id) where telephone_number like '%$phone%' and campaign_id in(22,28,29,32)  group by records.urn order by c1";
