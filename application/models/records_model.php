...
class Records_model extends CI_Model
{
    ...
    public function get_users($urn = "", $campaign_id = "")
    {
        ...
        elseif (empty($urn) && !empty($campaign_id)):
            ...
            $qry = "select users.user_id,name,user_email,user_telephone from users join users_to_campaigns using(user_id) join records using(campaign_id) join ownership using(urn) where user_status = 1 and urn = '$urn' and campaign_id in({$_SESSION['campaign_access']['list']})";
			$this->firephp->log($qry);
        endif;
        return $this->db->query($qry)->result_array();
    }
