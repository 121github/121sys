...
class Records_model extends CI_Model
{
    ...
    public function get_details($urn, $features)
    {
        ...
        if (in_array(2, $features)) {
            ...
            $from .= " left join companies com using(urn) left join company_addresses coma using(company_id) left join uk_postcodes com_pc on compc.postcode = coma.postcode left join company_telephone comt using(company_id) left join company_subsectors using(company_id) left join subsectors using(subsector_id) left join sectors using(sector_id)";
        }
