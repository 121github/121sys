...
<head>
    ...
    ?>
    <link rel="stylesheet" type="text/css" href="<?php echo minify($css_path, $css_files, $project_version) ?>"/>
    <?php $css_path = "assets";
    ...
    ?>
    <script src="<?php echo minify($js_path, $js_files, $project_version) ?>"></script>
...
<?php if (isset($_SESSION['user_id'])) { ?>
    ...
    <nav id="quick-actions-right" style="display:none; text-align:center; z-index: 110"
         class="mm-menu mm--horizontal mm-offcanvas">
...
<?php if (isset($_SESSION['user_id'])) {
    ...
    <script src="<?php echo minify($js_path, $js_files, $project_version) ?>"></script>
<?php
    $js_path = "custom_scripts";
    $js_custom_files = array();
    if (!empty($campaign_triggers)) {
        foreach ($campaign_triggers as $script) {
            $js_custom_files[] = $script['path'];
        }
?>
    <script src="<?php echo minify($js_path, $js_custom_files, $project_version) ?>"></script>
<?php } ?>
    <script type="text/javascript">
        modals.init();
