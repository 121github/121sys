...
class Flexicart_standard extends CI_Controller {
	...
	/**
	 * FLEXI CART DEMO
	 * Many of the functions within this controller load a custom model called 'orders_model' that has been created for the purposes of this demo.
	 * The 'orders_model' file is not part of flexi cart, it is included to demonstrate how some of the functions of flexi cart can be used.
	 */
	...
	function view_cart() 
	{
		...
		else if ($this->input->post('checkout'))
		{
			...
			{
				$msg ='The minimum order value of '.$this->flexi_cart->minimum_order().' has not been reached.';
				$this->flexi_cart->set_error_message($msg, 'public');
			...
			{
				$msg ='There are no items in the cart that can currently be shipped to the current shipping location.';
				$this->flexi_cart->set_error_message($msg, 'public');
				echo json_encode(array("success"=>false,"msg"=>$msg));
				exit;
			}
		...
		# $this->data['shipping_options'] = $this->orders_model->demo_manual_shipping_options(); 
		 ...
		 * #1: Replace the four "$this->data" arrays set above with "$this->data['shipping_options'] = $this->orders_model->demo_manual_shipping_options();".
		...
		*/
		...
		$this->load->view('flexicart/cart_view', $this->data);
	}
	...
	function update_cart()
	{
		...
		$this->load->model('orders_model');
		$this->orders_model->demo_update_cart();
	...
	function insert_link_item_to_cart($item_id = 0)
	{
		...
		$this->orders_model->demo_insert_link_item_to_cart($item_id);
	...
	function insert_form_item_to_cart($item_id = 0)
	{
		...
		$this->orders_model->demo_insert_form_item_to_cart($item_id);
	...
	function insert_ajax_link_item_to_cart($item_id = 0)
	{
		...
		$this->orders_model->demo_insert_ajax_link_item_to_cart($item_id);
	...
	{
		$this->load->model('orders_model');
		$this->orders_model->demo_insert_ajax_form_item_to_cart();
	...
	function insert_discount_item_to_cart($item_id = 0) 
	{
		...
		$this->orders_model->demo_insert_discount_item_to_cart($item_id);
	...
	function insert_database_item_to_cart($item_id = 0)
	{
		...
		$this->orders_model->demo_insert_database_item_to_cart($item_id);
	...
	function set_discount($discount_id = FALSE)
	{
		...
		$this->orders_model->demo_set_discount($discount_id);
	...
	function set_surcharge($surcharge_id = FALSE)
	{
		...
		$this->orders_model->demo_set_surcharge($surcharge_id);
	...
	function checkout() 
	{
		...
		if ($this->input->post('checkout'))
		{
			...
			$response = $this->orders_model->demo_save_order();

			if ($response)
			{
				// Attach the saved order number to the redirect url.
				$this->checkout_complete($this->flexi_cart->order_number());
				$url = $_SESSION['base'].'records/detail/'.$_SESSION['record_urn'];
				echo json_encode(array("success"=>true,"id"=>$this->flexi_cart->order_number(),"urn"=>$_SESSION['record_urn'],"url"=>$url));
				exit;
	...
	function load_cart_data($cart_data_id = 0) 
	{
		...
		$this->orders_model->demo_update_loaded_cart_data();
	...
	function misc_features() 
	{
		...
		{
			$this->orders_model->demo_update_tax();
		}
