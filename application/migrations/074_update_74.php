...
$this->db->query("INSERT ignore INTO `permissions` (`permission_id`, `permission_name`, `permission_group`) VALUES (NULL, 'admin shop', 'Admin')");
	$this->db->query("insert ignore into role_permissions values(1,(select permission_id from permissions where permission_name = 'admin shop'))");
		$this->db->query("insert ignore into role_permissions values(1,(select permission_id from permissions where permission_name = 'planner'))");
		$this->db->query("insert ignore into role_permissions values(1,(select permission_id from permissions where permission_name = 'record_options'))");
	}
