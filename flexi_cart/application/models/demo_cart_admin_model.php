...
class Demo_cart_admin_model extends CI_Model {
	...
	public function demo_get_item_categories(){
		$this->db->select("item_categories.item_category_id id,item_category_name name, if(`count` is null,'0',`count`) `count`",false);
			$this->db->join("(select item_category_id,count(*) count from item_subcategories group by item_category_id) b","b.item_category_id=item_categories.item_category_id","LEFT",FALSE);
	$this->db->group_by("item_categories.item_category_id");	
		return $this->db->get("item_categories")->result_array();	
	}
	
	public function demo_get_item_category($id){
		$this->db->where("item_category_id",$id);
		return $this->db->get("item_categories")->result_array();	
	}
	
	public function demo_get_item_subcategories($id=false){
		if($id){
		$this->db->where("item_category_id",$id);
		}
		return $this->db->get("item_subcategories")->result_array();	
	}
