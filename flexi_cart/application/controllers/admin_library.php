...
class Admin_library extends CI_Controller {
	...
	function __construct() 
	{
		...
		$this->load->vars('base_url', 'http://pro.121system.com/flexi_cart/');
		$this->load->vars('includes_dir', 'http://pro.121system.com/flexi_cart/includes/');
	...
	{		
		...
		$this->template->load('default', 'demo/admin_examples/dashboard_view', $this->data);
	}

	public function category_details(){
		$id = $this->input->post('id');
		$category = $this->demo_cart_admin_model->get_category($id);
		$category['subcategories'] = $this->demo_cart_admin_model->subcategories_in_category($id);	
	...
	{		
		...
		$this->data['categories'] = $this->demo_cart_admin_model->demo_get_item_categories();
		$this->data['subcategories'] = $this->demo_cart_admin_model->demo_get_item_subcategories();
		$this->data['javascript'] =  array('flexicart/categories.js?v' . time());
		$this->template->load('default', 'demo/admin_examples/config/categories', $this->data);
	}

	public function save_category(){
	$form = $this->input->post();
	$id = $this->demo_cart_admin_model->save_category($form);	
	if($this->input->post('subcategories')){
	$subcategories = $form['subcategories'];
	unset($form['subcategories']);
	if($subcategories){
		foreach($subcategories as $subcat_id){
	$this->demo_cart_admin_model->save_subcategory($subcat_id,$form['item_category_id']);	
		}
	}
	}
	...
	$categories = $this->demo_cart_admin_model->demo_get_item_categories();
	echo json_encode(array("success"=>true,"data"=>$categories));
	}
	
		public function get_subcategories(){
	$data = $this->demo_cart_admin_model->demo_get_item_subcategories($this->input->post('id'));
	...
	function items()
	{
		...
		$this->template->load('default', 'demo/admin_examples/items/items_view', $this->data);
	...
	function orders() 
	{
		...
		$this->template->load('default','demo/admin_examples/orders/orders_view', $this->data);
	}
	...
	function order_details($order_number) 
	{
		...
		$this->template->load('default','demo/admin_examples/orders/order_details_view', $this->data);
	}
