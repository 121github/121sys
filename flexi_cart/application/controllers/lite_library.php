...
class Lite_library extends CI_Controller {
		...
		function orders()
	{
		...
		$this->data['javascript'] =  array('flexicart/orders.js?v' . time());
		$this->template->load('default', 'demo/feature_examples/order_form', $this->data);

	 }
	 
	 function show_session(){
		 $flexicart = $this->session->all_userdata();
		echo "<pre>";
		print_r($flexicart['flexi_cart']);
		echo "<pre>";
	 }
	 
	 	function get_cart_quantities(){
			$flexicart = $this->session->userdata('flexi_cart');
			$data=array();
			foreach($flexicart['items'] as $k=>$cart_item){
				$data[] = array("item_id"=>$cart_item['id'],"quantity"=>$cart_item['quantity'])	;
			}
		...
		$data = $this->demo_cart_model->get_items($filter);
		$flexicart = $this->session->userdata('flexi_cart');
		foreach($data as $k=>$item_row){
			foreach($flexicart['items'] as $hash=>$cart_item){
				if($cart_item['id']==$item_row['item_id']){
				$data[$k]['quantity'] = $cart_item['quantity']	;
				}
			}
		}
		echo json_encode(array("success"=>true,"data"=>$data,"items"=>$flexicart['items']));
