    ...
    init: function() {
	 ...
	 categories: {
        ...
        load_categories: function() {
            $.ajax({
                url: helper.baseUrl + 'admin_library/get_categories',
                type: "POST",
                dataType: "JSON",
				beforeSend:function(){
				$('#form-container').hide();
				...
				var table = '<table class="table"><thead><tr><th>ID</th><th>Name</th><th>Subategories</th><th>Options</th></tr></thead><tbody>';
        ...
        edit: function(id) {
			$.ajax({
                url: helper.baseUrl + 'admin_library/category_details',
                type: "POST",
                dataType: "JSON",
				data: { id:id }
            }).done(function(response) {
			$('#form-container').find('[name="item_category_id"]').val(response.data.item_category_id);
			$('#form-container').find('[name="item_category_name"]').val(response.data.item_category_name);	
			$('#form-container').find('[name="item_category_decription"]').val(response.data.item_category_decription);
			if(response.data.subcategories.length>0){
			$('#subcat-container').show();
			$('#category-select').selectpicker('val',response.data.subcategories).selectpicker('refresh');
			} else {
        ...
        save: function() {
            $.ajax({
                url: helper.baseUrl + 'admin_library/save_category',
                type: "POST",
                dataType: "JSON",
                ...
                admin.categories.load_categories();
                admin.hide_edit_form();
                flashalert.success("Category saved");
            });
        },
        remove: function(id) {
            $.ajax({
                url: helper.baseUrl + 'admin_library/delete_category',
                type: "POST",
                dataType: "JSON",
                data: {
                    id: id
...
var modal = {
    delete_category: function(id) {
        ...
        $('#modal').modal({
            backdrop: 'static',
            keyboard: false
        }).find('.modal-body').text('Are you sure you want to delete this category?');
