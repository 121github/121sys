...
var maps = {
    initialize: function() {
        ...
        this.colour_by = null;
        this.lat = getCookie('location_error')==""?getCookie('lat'):maps.uk_lat;
        this.lng = getCookie('location_error')==""?getCookie('lng'):maps.uk_lng;
        var default_zoom = maps.default_zoom;
        this.myLatlng = new google.maps.LatLng(maps.lat, maps.lng);
		this.ukLatlng = new google.maps.LatLng(maps.uk_lat, maps.uk_lng);
        this.mapOptions = {
            zoom: default_zoom,
            center: maps.ukLatlng,
            mapTypeControl: true,
        ...
        $('#map-form').on('click', '.get-location-btn', function() {
        ...
        $('#map-form').on('click', '#show-uk', function() {
        ...
        $('#map-form').on('click', '.planner-travel-mode', function() {
		...
		//this loads the table for the first time when the map has finished loading
    ...
    showMap: function() {
        ...
        $(document).on('mouseenter', 'li .planner-title', function() {
        ...
        $(document).on('mouseleave', 'li .planner-title', function() {
    ...
    getDirections: function(origin, destination, waypoints, record_list_ord) {
        ...
        $('.planner-travel-mode').fadeTo("fast", 0.4);
        ...
        maps.directionsService.route(request, function(result, status) {
            if (status == google.maps.DirectionsStatus.OK) {
                ...
                $.each(result.routes[0].legs, function(index, route) {
                    if (page_name == 'planner') {
                        var data_route = {
                            ...
                            'start_lng': route.start_location.lng(),
                            'end_add': route.end_address,
                            'end_lat': route.end_location.lat(),
                            'end_lng': route.end_location.lng(),
                ...
                $('.route-info').html(
                    '<span>' + (Math.ceil((total_distance / 1000) / 1.2)) + ' miles - ' +
        ...
        maps.directionsDisplay.setOptions({
    ...
    addRecordMarker: function(value) {
        ...
        if (((view.planner_permission == true)) && (value.record_planner_id)) {
        ...
        if (view.planner_permission == true) {
            planner_info =
                '<b>Planner: </b>' +
    ...
    setMarker: function(marker) {
        ...
        google.maps.event.addListener(marker, 'mouseout', function() {
            $('.data-table tbody').find("[postcode='" + marker.postcode + "']").removeAttr('style');
            $('li').find(".planner-title[data-urn='" + marker.id + "']").removeAttr('style');
        });
