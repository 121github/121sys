	...
	roles: {
            ...
            $.ajax({
                ...
                $.each(response.data, function(i, val) {
                    if (response.data.length) {
                        $tbody.append("<tr><td class='role_id'>" + val.id + "</td><td class='role_name'>" + val.name + "</td><td class='landing_page'>" + val.landing_page + "</td><td class='timeout'>" + val.timeout + "</td><td><button class='btn btn-default btn-xs edit-btn'>Edit</button> <button class='btn btn-default btn-xs del-btn' item-id='" + val.id + "'>Delete</button></td></tr>");
                    }
                });
            });
        },
        ...
        edit: function($btn) {
            ...
            $('form').find('input[name="role_name"]').val(row.find('.role_name').text());
			 ...
			 $('form').find('input[name="timeout"]').val(row.find('.timeout').text());
            $.ajax({
