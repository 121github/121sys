...
var campaignoutcome = {
    init: function() {
        $('.daterange').daterangepicker({
            ...
            function(start, end, element) {
                ...
                campaignoutcome.campaignoutcome_panel()
            });
    ...
    campaignoutcome_panel: function(campaignoutcome) {
        $.ajax({
            url: helper.baseUrl + 'reports/campaignoutcome_data',
            type: "POST",
            dataType: "JSON",
            data: $('.filter-form').serialize()
        }).done(function(response) {
            var $row = "";
            $tbody = $('.campaignoutcome-data .ajax-table').find('tbody');
    		$tbody.empty();
            if (response.success) {
