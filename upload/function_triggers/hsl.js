...
planner_summary:function(data){
	...
	var table = "";
		...
		$.each(data.waypoints,function(date,waypoint){
			...
			table += "<tr><td>"+waypoint.start.uk_date+"</td><td>"+slots.apps+"/"+slots.max_apps+"</td><td>"+stats[5].distance.text+"</td><td>"+stats[5].duration.text+"</td><td><button class='btn btn-default btn-xs simulate' data-date='"+date+"' data-time='"+time+"' data-uk-date='"+waypoint.start.uk_date+"'>Simulate</button></td></tr>";
		});
